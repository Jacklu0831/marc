...
def initialize_kv(
    ...
    if separate_kv:
        ...
        dummy_input_ids = torch.arange(0, random_kv_ntokens - demon_start_idxs[0])[None, ...].to(accelerator.device)
        ...
        if random_kv == 'uniform':
            for layer_k, layer_v in past_key_values:
                uniform_layer_k = torch.rand((1, model.config.num_key_value_heads, random_kv_ntokens - demon_start_idxs[0], model.config.head_dim), device=accelerator.device, dtype=torch.float32)
                uniform_layer_v = torch.rand((1, model.config.num_key_value_heads, random_kv_ntokens - demon_start_idxs[0], model.config.head_dim), device=accelerator.device, dtype=torch.float32)
                layer_k[:, :, demon_start_idxs[0]:, :].copy_(uniform_layer_k)
                layer_v[:, :, demon_start_idxs[0]:, :].copy_(uniform_layer_v)
        ...
        for task in tqdm(process_tasks, desc='Task'):
            ...
            for eval_step, batch_idxs in enumerate(data_idxs):
                ...
                with accelerator.autocast():
                    ...
                    # Convert to Cache format
                    batch_past_key_values_cache = DynamicCache.from_legacy_cache(batch_past_key_values)
                    gen_tokens = model.generate(
                        input_ids=gen_input_ids,
                        attention_mask=gen_attention_mask,
                        past_key_values=batch_past_key_values_cache,
