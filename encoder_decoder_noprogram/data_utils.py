...
# def collate_fn_train(batch: List[int], dataset: TrainDataset, ntokens: int = 0, thinking_token_id: int = 666) -> Dict:
#     # ntokens indicates the length of thinking promgram, 0 means no thinking tokens
#     batch_size = len(batch)
#     del batch  # we don't use it directly
...
#             # label id for all except first pair
#             label_ids = torch.full(input_grid_ids.shape, -100, dtype=torch.int64)
#             # print(f"output grid ids shape:{output_grid_ids.shape}")
#             if pair_i == 0:
                
#                 # label_ids = torch.cat([label_ids, torch.full((output_grid_ids.shape[0],), -100, dtype=torch.int64)])
#                 label_ids = torch.cat([label_ids, torch.full(output_grid_ids.shape, -100, dtype=torch.int64)])
#             else:
#                 label_ids = torch.cat([label_ids, output_grid_ids])
#             # aggregate
...
#             pair_idx_to_label_ids.append(label_ids)
#             if ntokens !=0 :
#                 pair_idx_to_input_ids.append(torch.tensor([thinking_token_id] * ntokens))
#                 pair_idx_to_attention_mask.append(torch.full((ntokens,), 1, dtype=torch.int64))
#                 pair_idx_to_label_ids.append(torch.full((ntokens,), -100, dtype=torch.int64))
...
#         if len(input_ids)-required_num_pair*ntokens > dataset.max_seq_len:
#             continue
