...
def main():
    ...
    if args.ttt_weight_dir != None:
        ...
        for task_weight_dir in glob.glob(f"{ttt_weight_dir}/*"):
            ...
            if os.path.isdir(task_weight_dir) and len(task_name) == 8:
                ttt_path = os.path.join(task_weight_dir, f"lora_epoch_{args.ttt_weight_epoch}.pt")
                assert os.path.exists(ttt_path), ttt_path
                task_to_ttt_path[task_name] = ttt_path
        ...
        ttt_path = list(task_to_ttt_path.values())[0]
    ...
    # evaluate
    ce_loss, exact_acc, valid_grid, correct_grid_dim, token_acc, texts, votes, competition_sub_acc, competition_all_acc, ttt_provided = evaluate(
