...
class Prefix2PrefixProjection(nn.Module):
    ...
    def forward(
        ...
        if self.projection_type == "none":
            ...
            # clamping
            k_logvar = torch.clamp(k_logvar, min=-6, max=4)
            v_logvar = torch.clamp(v_logvar, min=-6, max=4)
...
def encoder_decoder_loss(
    ...
    total_loss = ce_loss + invar_loss_lambda * invar_loss + encoder_loss_lambda * encoder_loss + kl_loss_lambda * kl_loss
...
def decoder_loss(
    ...
    if conditioning_method == "prefix2prefix":
        ...
        if hasattr(decoder_module.model, "embed_tokens"):
            decoder_inputs_embeds = decoder_module.model.embed_tokens(decoder_input_ids) # nolora
        else:
            decoder_inputs_embeds = decoder_module.model.model.embed_tokens(decoder_input_ids) # lora
        if decoder_pad_side == "right":
        ...
        for batch_idxs in data_idx_iterator:
            ...
            with accelerator.autocast():
                ...
                if conditioning_method == "prefix2prefix":
                    ...
                    assert isinstance(predicted_program, torch.Tensor)
                    if hasattr(decoder_module.model, "embed_tokens"):
                        decoder_inputs_embeds = decoder_module.model.embed_tokens(dec_gen_ids) # nolora
                    else:
                        decoder_inputs_embeds = decoder_module.model.model.embed_tokens(dec_gen_ids) # lora
                    # pad decoder inputs embeds
...
def main():
    ...
    parser.add_argument("--linear_invar", action="store_true")
    ...
    train_tasks_dict = load_re_arc_from_data_dir(args.train_data_dir)
