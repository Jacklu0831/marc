...
def hidden_state_to_past_key_values(
    ...
    past_key_values = []
    assert len(model.transformer.h) == len(hidden_states)
    for x, blk in zip(hidden_states, model.transformer.h):
        # x = blk.ln_1(x.float()).to(torch.bfloat16)
        x = blk.ln_1(x)
        _, key, value = blk.attn.c_attn(x).split(blk.attn.split_size, dim=2)
        key = blk.attn._split_heads(key, blk.attn.num_heads, blk.attn.head_dim)
        value = blk.attn._split_heads(value, blk.attn.num_heads, blk.attn.head_dim)
...
def test_time_evaluate(
    ...
    with distributed_state.split_between_processes(dataset.tasks) as process_tasks:
        ...
        for task in tqdm(process_tasks, desc='Task'):
            ...
            # initialize kv
            start_time = time.time()
            hidden_states = initialize_hidden(
                model=model,
    ...
    assert len(init_hidden_time_list) == len(dataset.tasks), (len(init_hidden_time_list), len(dataset.tasks))
...
def run_gs(
    ...
    if ntokens != -1:
        ...
        assert len(hidden_states) == model.config.n_layer # type: ignore
