...
def remove_val_from_1dtokenized(tokenized: Dict, val: int) -> None:
...
class EvalDataset:
    ...
    def get_io_permuted_batches(self, batch_idxs: List[int]) -> Iterator[Tuple[List[Optional[Dict]], List[bool]]]:
        ...
        for permute_i in range(max_num_permutation):
            ...
            yield (
                [self.format_and_filter(task, permutation) for task, permutation in zip(avail_eval_tasks, permutations)],
                avail,
            )
