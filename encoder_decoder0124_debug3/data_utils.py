...
def parse_input_output_grids(s: str) -> List[Dict[str, List[List[int]]]]:
    # Split and strip lines, ignoring empty lines
    ...
    n = len(lines)
    while i < n:
        assert lines[i] == "input", f"Expected 'input' at line {i}, got '{lines[i]}' instead."
        i += 1
        input_height = int(lines[i]); i += 1
        input_width  = int(lines[i]); i += 1
        ...
        assert len(input_grid) == input_height, "Input grid height does not match specified dimension."
        for row in input_grid:
            assert len(row) == input_width, "Input grid width does not match specified dimension."
        # Expect the "output" token
        assert lines[i] == "output", f"Expected 'output' at line {i}, got '{lines[i]}' instead."
        i += 1
        # Read output dimensions
        output_height = int(lines[i]); i += 1
        output_width  = int(lines[i]); i += 1
        ...
        assert len(output_grid) == output_height, "Output grid height does not match specified dimension."
        for row in output_grid:
            assert len(row) == output_width, "Output grid width does not match specified dimension."
    ...
    while len(out_list) < batch_size:
        ...
        for task_i12 in range(2):
            ...
            if not dataset.debug_fixed_train_order:
                chosen_pairs = dataset.rng.choice(pairs_for_task, size=required_count, replace=False) # type: ignore
...
class EvalDataset:
    ...
    def format_and_filter(self, task: Task, permutation: Optional[List[int]] = None) -> Optional[Dict]:
        ...
        # visualize
        # visualize_task(Task(train_examples=train_examples, test_example=task.test_example), out_path="temp1.jpg")
