...
def main(
    ...
    n_sample: int = 1,
    experiment_folder: str = "experiments/tti/new/",
    formatter: str = "arclib.messagers.GPTTextMessageRepresenterV2",
    max_lora_rank: int = 64,
    include_n: List[int] = [1],
    permute_n: int = 2,
    new_format: bool = False,
    barc_format: bool = False,
    add_diff_format: bool = False,
    use_all_lora: bool = False,
    ids: List[str] = None,
    ...
    tasks = read_tasks_from_single_file(data_file, solution_file=solution_file, test=True)

    id_to_lora_path = {}
    # get lora paths and filter tasks if necessary
    if lora_checkpoints_folder is not None:
        id_to_lora_path = {}
        for lora_path in glob.glob(f"{lora_checkpoints_folder}/*/adapter_model.bin"):
            lora_id = lora_path.split("/")[-2]
            id_to_lora_path[lora_id] = lora_path
            lora_dir = os.path.dirname(lora_path)

    if ids is not None:
        tasks = [task for task in tasks if task.name.split("-")[0] in ids]
    ...
    # lora_path = f"{lora_checkpoints_folder}/{example_task_id}/"
    # abstract away
    ...
    engine = initialize_engine(
        ...
        max_lora_rank=lora_adapter_config.get("r", max_lora_rank),
        enable_lora=lora_checkpoints_folder is not None,
        enforce_eager=False,
    ...
    # save
    all_predictions_file = os.path.join(experiment_folder, "all_predictions.json")
    ...
    submission_file = os.path.join(experiment_folder, "submission.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ids", nargs="*", help="ids to evaluate"
    )
