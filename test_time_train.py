...
def main(experiment_folder: str = "experiments/ttt/new/",
    data_file: str = "/kaggle/input/arc-prize-2024/arc-agi_evaluation_challenges.json",
    num_tasks: Optional[int] = None,
    base_checkpoint: str = "checkpoints/pretrained/all_in_fix_final_checkpoints/",
    lora_checkpoints_folder: str = "checkpoints/ttt/all_in_fix_final_lora_clean/",
    quantization: Optional[str] = None,
    max_tokens: int = 8192,
    cpus: int = 64,
    lora_config: str = "configs/ttt/8B_lora_single_device.yaml",
    formatter: str = "arclib.messagers.GPTTextMessageRepresenterV2",
    ...
    base_checkpoint_dir: str = "checkpoints/pretrained/multi_format_model/",
    new_format: bool = False,
    barc_format: bool = False,
    ...
    arc_test_tasks = read_tasks_from_single_file(data_file, test=True)
    ...
    if ids is not None:
        arc_test_tasks = [task for task in arc_test_tasks if task.name.replace("-0", "") in ids]
    ...
    conf = _merge_yaml_and_cli_args(
        *TuneRecipeArgumentParser(
        ...
        ).parse_known_args(["--config={}".format(lora_config)])
    )

    # Update conf with argparse settings
    conf.dataset.unmask_outputs = unmask_outputs
    conf.dataset.train_on_input = train_on_input
    conf.epochs = epochs
    conf.batch_size = batch_size
    conf.gradient_accumulation_steps = gradient_accumulation_steps
    conf.optimizer.lr = learning_rate
    conf.compile = False  # we will do it ourselves
    conf.model.lora_rank = lora_rank
    conf.model.lora_alpha = lora_alpha
    conf.model.lora_attn_modules = lora_attn_modules
    conf.model.apply_lora_to_mlp = lora_to_mlp
    conf.model.lora_dropout = lora_dropout
    conf.checkpointer.checkpoint_dir = base_checkpoint_dir
    conf.seed = seed

    if "llama3_2" not in conf.model._component_:
        conf.model.apply_lora_to_output = lora_to_output
    else:
    ...
    for task, task_train_data in zip(arc_test_tasks, data):
        ...
        os.makedirs(f"{experiment_folder}/{task_id}", exist_ok=True)
        ...
        with open(
            ...
            f"{experiment_folder}/{task_id}/td_True_ttd_False_ttdwa_False_ad_True_trd_False.jsonl",
            "w",
        ) as dst:
    ...
    for task in arc_test_tasks:
        ...
        try:
            adapter_path = f"{experiment_folder}/{task_id}/adapter_model.bin"
            # if exist continue
            if os.path.exists(adapter_path):
                print(f"Adapter for {task_id} already exists, skipping")
                continue
            test_file = f"{experiment_folder}/{task_id}/td_True_ttd_False_ttdwa_False_ad_True_trd_False.jsonl"
            ...
            saved_dict = None
            adapter_config_path = f"{experiment_folder}/{task_id}/adapter_config.json"
            save_adapter_config(
                adapter_config_path,
                base_checkpoint_dir,
                lora_rank=lora_rank,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ids", type=str, nargs="+", default=None, help="Task ids to process"
    )
