...
model = AutoModelForCausalLM.from_pretrained(args.pretrained_checkpoint, torch_dtype=torch.bfloat16, cache_dir=f'{args.pretrained_checkpoint}_cache').to("cuda")

# Define prompt tuning configuration
if not args.use_base_model:
    prompt_tuning_config = PromptTuningConfig(
        task_type="CAUSAL_LM",  # Type of task (causal language modeling here)
        num_virtual_tokens=args.num_virtual_tokens,  # Number of virtual prompt tokens
        token_dim=model.config.hidden_size,  # Hidden size of the model
        inference_mode=True,
    )
