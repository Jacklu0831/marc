...
def initialize_kv(
    ...
    if separate_kv:
        ...
        past_key_values_transform = torch.nn.Sequential(
            ...
            torch.nn.Tanh(),
            torch.nn.Linear(token_dim, model.config.n_layer * 2 * model.config.n_embd),
    ...
    return past_key_values_seed, past_key_values_transform # type: ignore
...
def test_time_evaluate(
    ...
    with distributed_state.split_between_processes(dataset.tasks) as process_tasks:
        ...
        for task in tqdm(process_tasks, desc='Task'):
            ...
            # initialize kv
            start_time = time.time()
            past_key_values_seed, past_key_values_transform = initialize_kv(
                model=model,
            ...
            if gs_epochs > 0:
                with accelerator.no_sync(model):
                    ...
                    model, past_key_values_seed, past_key_values_transform, gs_num_data, gs_num_params, attn_logger, fisher_vals = run_gs(
                        demonstration_pairs=dataset.task_to_demonstrations[task],
                        eval_dataset=dataset,
                        accelerator=accelerator,
                        model=model,
                    ...
                    past_key_values_flat = past_key_values_flat.view(model.config.n_layer, 2, 1, model.config.n_head, -1, model.config.n_embd // model.config.n_head)
...
def run_gs(
    ...
    kv_len_before_dropout = past_key_values_seed.shape[0]
    ...
    return model, past_key_values_seed, past_key_values_transform, len(gs_dataset), num_params, attn_logger, ret_fisher_vals
