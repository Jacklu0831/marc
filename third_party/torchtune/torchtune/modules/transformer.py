...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        # jack: this is for prompt tuning
        if prompt_encoder is not None:
            batch_size = tokens.shape[0]
            prompt_tokens = prompt_tokens.unsqueeze(0).expand(batch_size, -1).to(prompt_encoder.embedding.weight.device)
            prompts = prompt_encoder(prompt_tokens[:1])
            prompts = prompts.repeat(batch_size, 1, 1)
            prompts = prompts.to(h.dtype)
